...
#let drawAxesLines(tamp, lengths:1, strokes:gray) = {
...
#let drawPlaneXY(tamp, steps:0.1, lengths:1, backlengths:1, labels:none, stroke:silver+0.5pt) = {
	if type(steps) != array { steps = (steps, steps) }
	if type(lengths) != array { lengths = (lengths, lengths) }
	if type(backlengths) != array { backlengths = (backlengths, backlengths) }
	let imax = lengths.at(0)/steps.at(0)
	let jmax = lengths.at(1)/steps.at(1)
	let i = -backlengths.at(0)
	while i <= imax {
		place(line(
			start:tamp((i, -backlengths.at(1))),
			end:tamp((i, jmax)),
			stroke:stroke,
		))
		i += steps.at(0)
	}
	let j = -backlengths.at(1)
	while j <= jmax {
		place(line(
			start:tamp((-backlengths.at(0), j)),
			end:tamp((imax, j)),
			stroke:stroke,
		))
		j += steps.at(0)
...
#let drawAxes(
	tamp,
	lengths:1,
	backlengths:auto, // auto (int, int, int)
	axes:gray,
	grid:silver+0.5pt,
	gridSteps:1,
	labels:($x$, $y$, $z$), // none, (string, string, string)
	labelAxisOffsets:0,
	labelOffsets2d:(-0.25em, -0.5em),
) = {
	if type(lengths) != array { lengths = (lengths, lengths, lengths) }
	if type(backlengths) != array and backlengths != auto { backlengths = (backlengths, backlengths, backlengths) }
	if grid != none {
		drawPlaneXY(tamp, lengths:lengths, backlengths:backlengths, steps:gridSteps, stroke:grid)
	}
	if axes != none {
		drawAxesLines(tamp, lengths:lengths, strokes:axes)
		if (backlengths == auto) {
			drawAxesLines(tamp, lengths:vec.scale(lengths, -1), strokes:axes)
		} else {
			drawAxesLines(tamp, lengths:vec.scale(backlengths, -1), strokes:axes)
		}
	}
	if labels != none {
		drawAxesLabels(tamp, lengths:lengths, extras:labelAxisOffsets, offsets:labelOffsets2d)
	}
}
...
#let drawLine(tamp, p1, p2, stroke:black) = {
	place(line(
		start:tamp(p1),
		end:tamp(p2),
		stroke:stroke
...
#let polygon(tamp, points, stroke:black, vertices:none) = {
	let i = 1
	while i < points.len() {
		drawLine(tamp, points.at(i - 1), points.at(i), stroke:stroke)
	}
	//TODO: vertices
}

#let drawCurve(
	tamp, r, // r is a vector function of 1 variable
	interval:(0,1),
	steps:10,
...
#let drawSurface(
	tamp, r, // r is a vector function of two variables
	interval:( (0,1), (0,1) ),
	steps:10,
	stroke:none,
